	...
	const bool debug)
{
  ...
  torch::Tensor count_accum = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor weight_accum = torch::full({P}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		geovalues.contiguous().data<float>(), 
		...
		count_accum.contiguous().data<int>(), 
		weight_accum.contiguous().data<float>(), 
		debug);
  }
  return std::make_tuple(rendered, out_color, out_others, radii, count_accum, weight_accum, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dgeovalues = torch::zeros({P, 1}, means3D.options());
  ...
  {  
	  ...
	  dL_dgeovalues.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dgeovalues, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations);
}
